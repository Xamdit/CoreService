...
class Expenses_model extends App_Model
{
    ...
    public function get_expenses_total($data)
    {
        ...
        for ($i = 1; $i <= 5; $i++) {
            ...
            switch ($i) {
                ...
                case 4:
                    ...
                    $this->db->where('invoiceid IN (SELECT invoiceid FROM ' .  'invoices WHERE status=2 AND id=' .  'expenses.invoiceid)');
            ...
            $all_expenses = $this->db->get( 'expenses')->result_array();
            $_total_all   = [];
            $cached_taxes = [];
        ...
        foreach($all_expenses as $expense){
        $_total = 0;
        if(total_rows('invoices',array('status'=>2,'id'=>$expense['invoiceid'])) > 0){
        $_total = $expense['amount'];
        if($expense['tax'] != 0){
        $tax = get_tax_by_id($expense['tax']);
        $_total += ($_total / 100 * $tax->taxrate);
        ...
        foreach($all_expenses as $expense){
        $_total = 0;
        if(total_rows('invoices','status NOT IN(2,5) AND id ='.$expense['invoiceid']) > 0){
    ...
    public function delete($id, $simpleDelete = false)
    {
        ...
        if ($this->db->affected_rows() > 0) {
            ...
            $this->db->where('recurring_from', $id);
            $this->db->update( 'expenses', ['recurring_from' => null]);
    ...
    public function convert_to_invoice($id, $draft_invoice = false, $params = [])
    {
        ...
        if (db.get_option('invoice_due_after') != 0) {
            $new_invoice_data['duedate'] = _d(date('Y-m-d', strtotime('+' . db.get_option('invoice_due_after') . ' DAY', strtotime($invoice_date))));
        }
        ...
        if ($invoiceid) {
            ...
            if (is_custom_fields_smart_transfer_enabled()) {
                ...
                $cfExpenses = $this->db->get( 'customfields')->result_array();
                foreach ($cfExpenses as $field) {
                    ...
                    if (isset($tmpSlug[1])) {
                        ...
                        $cfTransfer = $this->db->get( 'customfields')->result_array();
    ...
    public function get_category($id = '')
    {
        if (is_numeric($id)) {
            ...
            return $this->db->get( 'expenses_categories')->row();
        }
        $this->db->order_by('name', 'asc');

        return $this->db->get( 'expenses_categories')->result_array();
    }
    ...
    public function add_category($data)
    {
        ...
        $this->db->insert( 'expenses_categories', $data);
        $insert_id = $this->db->insert_id();
        if ($insert_id) {
    ...
    {
        if (is_reference_in_table('category',  'expenses', $id)) {
            return [
                'referenced' => true,
            ];
        }
        $this->db->where('id', $id);
        $this->db->delete( 'expenses_categories');
        if ($this->db->affected_rows() > 0) {
    ...
    {
        return $this->db->query('SELECT DISTINCT(YEAR(date)) as year FROM ' .  'expenses ORDER by year DESC')->result_array();
    }
}
