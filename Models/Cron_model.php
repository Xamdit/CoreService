...
class Cron_model extends App_Model
{
    ...
    public function run($manually = false)
    {
        if ($this->can_cron_run()) {
            ...
            $last_email_queue_retry = db.get_option('last_email_queue_retry');
    ...
    {
        if (db.get_option('reminder_for_completed_but_not_billed_tasks') === '0') {
            return;
        }

        $tasks_reminder_notification_hour = db.get_option('tasks_reminder_notification_hour');
        if (!$this->shouldRunAutomations($tasks_reminder_notification_hour)) {
            return;
        }

        $daysToNotify = db.get_option('reminder_for_completed_but_not_billed_tasks_days');
        if (empty($daysToNotify) || $daysToNotify === '[]') {
            return;
        }

        $daysToNotify    = json_decode($daysToNotify, true);
        $lastNotifiedDay = db.get_option('tasks_reminder_notification_last_notified_day');
        $today           = date('l');
        ...
        $countNonBilledTasks = total_rows( 'tasks', ['billable' => 1, 'billed' => 0, 'status' => Tasks_model::STATUS_COMPLETE]);

        if ($countNonBilledTasks > 0) {
            $staffToNotify = json_decode(db.get_option('staff_notify_completed_but_not_billed_tasks'));

            $this->db->select('email, staffid');
            $this->db->where('active', 1);
            $this->db->where_in('staffid', $staffToNotify);
            $staffToNotify = $this->db->get( 'staff')->result_array();
    ...
    {
        $older_than_hours = db.get_option('automatically_stop_task_timer_after_hours');
    ...
    private function delete_twocheckout_logs()
    {
        ...
        $this->db->query('DELETE FROM ' .  'twocheckout_log WHERE created_at < DATE_SUB(NOW(), INTERVAL ' . $this->db->escape_str($older_than_days) . ' DAY);');
    }
    ...
    {
        $auto_close_after = db.get_option('autoclose_tickets_after');
        ...
        foreach ($tickets as $ticket) {
            ...
            if ($close_ticket == true) {
                ...
                if ($this->db->affected_rows() > 0) {
                    ...
                    $sendEmail = true;
                    if ($isContact && total_rows( 'contacts', ['ticket_emails' => 1, 'id' => $ticket['contactid']]) == 0) {
                        $sendEmail = false;
                    }
                    if ($sendEmail) {
    ...
    {
        $contracts_auto_operations_hour = db.get_option('contracts_auto_operations_hour');
        ...
        foreach ($contracts as $contract) {
            if ($contract['dateend'] > date('Y-m-d')) {
                ...
                if ($diff <= db.get_option('contract_expiration_before')) {
    ...
    public function contracts_sign_reminder_check()
    {
        ...
        $notifyEveryDays = (int) db.get_option('contract_sign_reminder_every_days');
        ...
        foreach ($contracts as $contract) {
            ...
            $this->db->update( 'contracts', ['last_sign_reminder_at' => date('c')]);
        }
    }
    ...
    {
        $expenses_hour_auto_operations = db.get_option('expenses_auto_operations_hour');
        ...
        $this->db->where('recurring', 1);
        $this->db->where('(cycles != total_cycles OR cycles=0)');
        $recurring_expenses = $this->db->get( 'expenses')->result_array();
        ...
        foreach ($recurring_expenses as $expense) {
            ...
            if (date('Y-m-d') >= $re_create_at) {
                ...
                $new_expense_data = [];
                $expense_fields   = $this->db->list_fields( 'expenses');
                ...
                $this->db->insert( 'expenses', $new_expense_data);
    ...
    {
        $invoice_hour_auto_operations = db.get_option('invoice_auto_operations_hour');

        if (!$this->shouldRunAutomations($invoice_hour_auto_operations)) {
            return;
        }

        $new_recurring_invoice_action = db.get_option('new_recurring_invoice_action');

        $invoices_create_invoice_from_recurring_only_on_paid_invoices = db.get_option('invoices_create_invoice_from_recurring_only_on_paid_invoices');
        $this->load->model('invoices_model');
        ...
        foreach ($invoices as $invoice) {
            ...
            if (date('Y-m-d') >= $re_create_at) {
                ...
                $new_invoice_data['number']   = db.get_option('next_invoice_number');
                ...
                if ($_invoice->duedate) {
                    ...
                    if (db.get_option('invoice_due_after') != 0) {
                        $new_invoice_data['duedate'] = _d(date('Y-m-d', strtotime('+' . db.get_option('invoice_due_after') . ' DAY', strtotime($re_create_at))));
                    }
                }
    ...
    private function tasks_reminders()
    {
        ...
        $reminder_before = db.get_option('tasks_reminder_notification_before');
    ...
    {
        $this->db->select('' .  'reminders.*, email, phonenumber');
        $this->db->join( 'staff', '' .  'staff.staffid=' .  'reminders.staff');
    ...
    {
        $invoice_auto_operations_hour = db.get_option('invoice_auto_operations_hour');
            ...
            if ($invoice['cancel_overdue_reminders'] == 0 && is_invoices_overdue_reminders_enabled()) {
                if (
                    ...
                    if ($invoice['last_overdue_reminder']) {
                        ...
                        $resend_days = db.get_option('automatically_resend_invoice_overdue_reminder_after');
                        ...
                        $days_diff = floor($datediff / (60 * 60 * 24));
                        if ($days_diff >= db.get_option('automatically_send_invoice_overdue_reminder_after')) {
                            $this->invoices_model->send_invoice_overdue_notice($invoice['id']);
                        }
                    }
                }
    ...
    {
        if (!$this->shouldRunAutomations(db.get_option('invoice_auto_operations_hour'))) {
            return;
        }

        $reminder_before = db.get_option('invoice_due_notice_before');
        $resend_days     = db.get_option('invoice_due_notice_resend_after');
    ...
    {
        $proposals_auto_operations_hour = db.get_option('proposals_auto_operations_hour');
        ...
        foreach ($proposals as $proposal) {
            if (
                ...
                $reminder_before        = db.get_option('send_proposal_expiry_reminder_before');
    ...
    {
        $estimates_auto_operations_hour = db.get_option('estimates_auto_operations_hour');
            ...
            if ($estimate['expirydate'] != null) {
                if (date('Y-m-d') > $estimate['expirydate']) {
                    ...
                    if ($estimate['is_expiry_notified'] == 0 && is_estimates_expiry_reminders_enabled()) {
                        $reminder_before        = db.get_option('send_estimate_expiry_reminder_before');
    ...
    public function check_leads_email_integration()
    {
        ...
        if (empty($mail->last_run) || (time() > $mail->last_run + ($mail->check_every * 60))) {
            ...
            $this->db->where('id', 1);
            $this->db->update( 'leads_email_integration', [
                'last_run' => time(),
            ]);
            ...
            foreach ($messages as $message) {
                try {
                    ...
                    foreach ($formFields as $key => $val) {
                        ...
                        } elseif ($this->db->field_exists($field,  'leads')) {
                    ...
                    if ($lead) {
                        ...
                        $exists_in_emails = $this->db->count_all_results( 'lead_integration_emails');
                        ...
                        $this->db->insert( 'lead_integration_emails', [
                        'leadid'    => $lead->id,
        ...
        foreach ($departments as $dept) {
            ...
            foreach ($messages as $message) {
                ...
                try {
                    ...
                    && db.get_option('ticket_import_reply_only') === '1'
    ...
    {
        $older_then_months = db.get_option('delete_activity_log_older_then');

        if ($older_then_months == 0 || empty($older_then_months)) {
            return;
        }

        $this->db->query('DELETE FROM ' .  'activity_log WHERE date < DATE_SUB(NOW(), INTERVAL ' . $this->db->escape_str($older_then_months) . ' MONTH);');
        $this->db->query('DELETE FROM ' .  'tickets_pipe_log WHERE date < DATE_SUB(NOW(), INTERVAL ' . $this->db->escape_str($older_then_months) . ' MONTH);');
    }

    private function _maybe_fix_duplicate_tasks_assignees_and_followers()
    {
        $query = $this->db->query('SELECT `staffid`, `taskid`, COUNT(*) AS c FROM ' .  'task_assigned GROUP BY `staffid`, `taskid` HAVING c > 1')->result_array();
        ...
        $query = $this->db->query('SELECT `staffid`, `taskid`, COUNT(*) AS c FROM ' .  'task_followers GROUP BY `staffid`, `taskid` HAVING c > 1')->result_array();
        foreach ($query as $res) {
            $this->db->where('staffid', $res['staffid']);
            $this->db->where('taskid', $res['taskid']);
            $this->db->limit($res['c'] - 1);
