...
class Credit_notes_model extends App_Model
{
    ...
    public function get_available_creditable_invoices($credit_note_id)
    {
        ...
        $credit_note = $this->db->get( 'creditnotes')->row();

        $this->db->select('' .  'invoices.id as id, status, total, date, ' .  'currencies.name as currency_name');
    ...
    */
    ...
    {
        $this->db->select('*,' .  'currencies.id as currencyid, ' .  'creditnotes.id as id, ' .  'currencies.name as currency_name');
        $this->db->from( 'creditnotes');
        $this->db->join( 'currencies', '' .  'currencies.id = ' .  'creditnotes.currency', 'left');
        $this->db->where($where);

        if (is_numeric($id)) {
            $this->db->where( 'creditnotes.id', $id);
    ...
    public function add($data)
    {
        ...
        $data['prefix']        = db.get_option('credit_note_prefix');
        $data['number_format'] = db.get_option('credit_note_number_format');
        ...
        if ($insert_id) {
            ...
            update_sales_total_tax_column($insert_id, 'credit_note',  'creditnotes');
    ...
    public function update($data, $id)
    {
        ...
        $this->db->where('id', $id);
        $this->db->update( 'creditnotes', $data);
        ...
        if ($affectedRows > 0) {
            ...
            update_sales_total_tax_column($id, 'credit_note',  'creditnotes');
        }
    ...
    */
    public function delete($id, $simpleDelete = false)
    {
        ...
        $this->db->where('id', $id);
        $this->db->delete( 'creditnotes');
        if ($this->db->affected_rows() > 0) {
            $current_credit_note_number = db.get_option('next_credit_note_number');
            ...
            $this->db->where('relid IN (SELECT id from ' .  'itemable WHERE rel_type="credit_note" AND rel_id="' . $this->db->escape_str($id) . '")');
            ...
            $this->db->delete( 'creditnote_refunds');
    ...
    {
        $this->db->where('id', $id);
        $this->db->update( 'creditnotes', ['status' => $status]);
    ...
    public function total_remaining_credits_by_customer($customer_id)
    {
        ...
        $credits = $this->db->get( 'creditnotes')->result_array();
    ...
    private function calc_remaining_credits($credits)
    {
        ...
        if (count($credits_ids) > 0) {
            ...
            $applied_credits = $this->db->get( 'credits')->result_array();
    ...
    public function credit_note_from_invoice($invoice_id)
    {
        ...
        $new_credit_note_data['number']   = db.get_option('next_credit_note_number');
    ...
    public function create_refund($id, $data)
    {
        ...
        $this->db->insert( 'creditnote_refunds', [
    ...
    public function edit_refund($id, $data)
    {
        ...
        $this->db->where('id', $id);
        $this->db->update( 'creditnote_refunds', [
            'refunded_on'  => $data['refunded_on'],
            'payment_mode' => $data['payment_mode'],
            'amount'       => $data['amount'],
            'note'         => nl2br($data['note']),
    ...
    public function get_refund($id)
    {
        ...
        return $this->db->get( 'creditnote_refunds')->row();
    }

    public function get_refunds($credit_note_id)
    {
        $this->db->select(prefixed_table_fields_array( 'creditnote_refunds', true) . ',' .  'payment_modes.id as payment_mode_id, ' .  'payment_modes.name as payment_mode_name');
        $this->db->where('credit_note_id', $credit_note_id);

        $this->db->join( 'payment_modes',  'payment_modes.id = ' .  'creditnote_refunds.payment_mode', 'left');

        $this->db->order_by('refunded_on', 'desc');

        $refunds = $this->db->get( 'creditnote_refunds')->result_array();
    ...
    {
        return sum_from_table( 'creditnote_refunds', [
                'field' => 'amount',
    ...
    public function apply_credits($id, $data)
    {
        ...
        if ($insert_id) {
            ...
            $invoice = $this->db->select('id,status')->where('id', $data['invoice_id'])->get( 'invoices')->row();
            ...
            $this->db->select( 'currencies.name as currency_name');
            $this->db->join( 'currencies', '' .  'currencies.id = ' .  'invoices.currency');
            $this->db->where( 'invoices.id', $data['invoice_id']);
    ...
    public function update_credit_note_status($id)
    {
        ...
        if ($total_credits_used || $total_refunds_by_credit_note) {
            ...
            $this->db->where('id', $id);
            $credit = $this->db->get( 'creditnotes')->row();
    ...
    public function get_applied_invoice_credits($invoice_id)
    {
        ...
        return $this->db->get( 'credits')->result_array();
    }
    ...
    {
        return $this->db->query('SELECT DISTINCT(YEAR(date)) as year FROM ' .  'creditnotes ORDER BY year DESC')->result_array();
    }
