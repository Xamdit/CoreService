...
class Contract_types_model extends App_Model
{
    ...
    */
    ...
    {
        $this->db->insert('contracts_types', $data);
    ...
    {
        $this->db->where('id', $id);
        $this->db->update('contracts_types', $data);
    ...
    public function get($id = '')
    {
        if (is_numeric($id)) {
            ...
            return $this->db->get('contracts_types')->row();
        }

        $types = $this->app_object_cache->get('contract-types');

        if (!$types && !is_array($types)) {
            $types = $this->db->get('contracts_types')->result_array();
    ...
    {
        if (is_reference_in_table('contract_type', 'contracts', $id)) {
            return [
                'referenced' => true,
            ];
        }
        $this->db->where('id', $id);
        $this->db->delete('contracts_types');
        if ($this->db->affected_rows() > 0) {
    ...
    public function get_chart_data()
    {
        ...
        foreach ($types as $type) {
            ...
            $total_rows = total_rows('contracts', $total_rows_where);
    ...
    public function get_values_chart_data()
    {
        ...
        foreach ($types as $type) {
            ...
            $total = sum_from_table('contracts', $where);
            if ($total == null) {
                $total = 0;
            }
