...
class Contracts_model extends App_Model
{
    ...
    {
        $this->db->select('*,' .  'contracts_types.name as type_name,' .  'contracts.id as id, ' .  'contracts.addedfrom');
        $this->db->where($where);
        $this->db->join( 'contracts_types', '' .  'contracts_types.id = ' .  'contracts.contract_type', 'left');
        $this->db->join( 'clients', '' .  'clients.userid = ' .  'contracts.client');
        if (is_numeric($id)) {
            $this->db->where( 'contracts.id', $id);
            $contract = $this->db->get( 'contracts')->row();
            if ($contract) {
    ...
    {
        return $this->db->query('SELECT DISTINCT(YEAR(datestart)) as year FROM ' .  'contracts')->result_array();
    }
    ...
    public function clear_signature($id)
    {
        ...
        if ($contract) {
            $this->db->where('id', $id);
            $this->db->update( 'contracts', array_merge(get_acceptance_info_array(true), ['signed' => 0]));
    ...
    */
    public function add_comment($data, $client = false)
    {
        ...
        if ($insert_id) {
            ...
            if ($client == true) {
                ...
                $staff_contract = $this->db->get( 'staff')->result_array();
    ...
    {
        $this->db->where('id', $id);
        $this->db->update( 'contract_comments', [
    ...
    public function get_comments($id)
    {
        ...
        return $this->db->get( 'contract_comments')->result_array();
    }
    ...
    public function get_comment($id)
    {
        ...
        return $this->db->get( 'contract_comments')->row();
    }
    ...
    public function remove_comment($id)
    {
        ...
        $this->db->where('id', $id);
        $this->db->delete( 'contract_comments');
        if ($this->db->affected_rows() > 0) {
    ...
    public function delete($id)
    {
        ...
        if ($this->db->affected_rows() > 0) {
            ...
            $this->db->delete( 'contract_renewals');
            // Get related tasks
            $this->db->where('rel_type', 'contract');
            $this->db->where('rel_id', $id);
    ...
    public function renew($data)
    {
        ...
        $this->db->insert( 'contract_renewals', $data);
        $insert_id = $this->db->insert_id();
        if ($insert_id) {
            ...
            $this->db->update( 'contracts', $_data);
            if ($this->db->affected_rows() > 0) {
    ...
    public function delete_renewal($id, $contractid)
    {
        ...
        $this->db->select('id')->from( 'contract_renewals')->where('contractid', $contractid)->order_by('id', 'desc')->limit(1);
        $query                 = $this->db->get();
        ...
        if ($last_contract_renewal == $id) {
            $is_last = true;
            $this->db->where('id', $id);
            $original_renewal = $this->db->get( 'contract_renewals')->row();
        }
    ...
    public function get_contract_renewal_history($id)
    {
        ...
        return $this->db->get( 'contract_renewals')->result_array();
    }
