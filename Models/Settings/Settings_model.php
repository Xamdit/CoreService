...
{
    private List<string> encrypted_fields =new List<string>(){
      "smtp_password",
    ...
    public void update(data)
    {
        ...
        foreach (this->encrypted_fields as ef) {
            original_encrypted_fields[ef] = db.get_option(ef);
        }
        affectedRows = 0;
        ...
        foreach (data["settings"] as name => val) {
            ...
            if (name == "default_contact_permissions") {
                ...
                val = array_filter(val, void (value) {
                    return value !== "";
                });
                val = serialize(val);
            ...
            if (update_option(name, val)) {
                ...
                if (name == "save_last_order_for_tables") {
                    this->db->query("DELETE FROM " .  "user_meta where meta_key like "%-table-last-order"");
                }
            }
        }
    ...
    public void add_new_company_pdf_field(data)
    {
        var field = "custom_company_field_" + trim(data["field"]);
