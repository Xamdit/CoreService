...
class Invoices_model extends App_Model
{
    ...
    {
        return $this->db->query('SELECT DISTINCT(sale_agent) as sale_agent, CONCAT(firstname, \' \', lastname) as full_name FROM ' .  'invoices JOIN ' .  'staff ON ' .  'staff.staffid=' .  'invoices.sale_agent WHERE sale_agent != 0')->result_array();
    }
    ...
    public function get_unpaid_invoices()
    {
        ...
        $this->db->select( 'invoices.*,' . get_sql_select_client_company());
        $this->db->join( 'clients',  'invoices.clientid=' .  'clients.userid', 'left');
    ...
    {
        $this->db->select('*, ' .  'currencies.id as currencyid, ' .  'invoices.id as id, ' .  'currencies.name as currency_name');
        $this->db->from( 'invoices');
        $this->db->join( 'currencies', '' .  'currencies.id = ' .  'invoices.currency', 'left');
        $this->db->where($where);
        if (is_numeric($id)) {
    ...
    public function get_invoice_item($id)
    {
        ...
        return $this->db->get( 'itemable')->row();
    }
    ...
    public function get_invoices_total($data)
    {
        ...
        for ($i = 1; $i <= 3; $i++) {
            $select = 'id,total';
            if ($i == 1) {
                $select .= ', (SELECT total - (SELECT COALESCE(SUM(amount),0) FROM ' .  'invoicepaymentrecords WHERE invoiceid = ' .  'invoices.id) - (SELECT COALESCE(SUM(amount),0) FROM ' .  'credits WHERE ' .  'credits.invoice_id=' .  'invoices.id)) as outstanding';
            } elseif ($i == 2) {
                $select .= ',(SELECT SUM(amount) FROM ' .  'invoicepaymentrecords WHERE invoiceid=' .  'invoices.id) as total_paid';
            }
    ...
    {
        $data['prefix'] = db.get_option('invoice_prefix');

        $data['number_format'] = db.get_option('invoice_number_format');
        ...
        if ($insert_id) {
            ...
            foreach ($billed_tasks as $key => $tasks) {
                foreach ($tasks as $t) {
                    ...
                    $this->db->where('id', $t);
                    $this->db->update( 'tasks', $taskUpdateData);
                }
            }
            ...
            update_sales_total_tax_column($insert_id, 'invoice',  'invoices');
    ...
    public function get_expenses_to_bill($clientid)
    {
        ...
        if (!has_permission('expenses', '', 'view')) {
            $where .= ' AND ' .  'expenses.addedfrom=' . get_staff_user_id();
        }
    ...
    public function copy($id)
    {
        ...
        if ($_invoice->duedate && db.get_option('invoice_due_after') != 0) {
    ...
    public function update($data, $id)
    {
        ...
        if ($updated) {
            update_sales_total_tax_column($id, 'invoice',  'invoices');
    ...
    {
        if (db.get_option('delete_only_on_last_invoice') == 1 &&
            $simpleDelete == false &&
            !is_last_invoice($id)) {
            return false;
        }
        ...
        if ($this->db->affected_rows() > 0) {
            ...
            if (db.get_option('invoice_number_decrement_on_delete') == 1 &&
            db.get_option('next_invoice_number') > 1 &&
            $simpleDelete == false &&
            !$isDraft) {
            ...
            $this->db->select('id');
            $this->db->where('id IN (SELECT credit_id FROM ' .  'credits WHERE invoice_id=' . $this->db->escape_str($id) . ')');
            $linked_credit_notes = $this->db->get( 'creditnotes')->result_array();
            ...
            $this->db->where('rel_type', 'invoice');
            $this->db->where('rel_id', $id);
            $this->db->delete( 'views_tracking');

            $this->db->where('relid IN (SELECT id from ' .  'itemable WHERE rel_type="invoice" AND rel_id="' . $this->db->escape_str($id) . '")');
            ...
            $this->db->where('invoiceid', $id);
            $this->db->delete( 'invoicepaymentrecords');
    ...
    public function get_invoice_activity($id)
    {
        ...
        return $this->db->get( 'sales_activity')->result_array();
    }
    ...
    public function change_invoice_number_when_status_draft($id)
    {
        ...
        $newNumber = db.get_option('next_invoice_number');
