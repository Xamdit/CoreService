...
class Reports_model extends App_Model
{
    ...
    {
        $result      = $this->db->query('select last_status_change from ' .  'leads where MONTH(last_status_change) = ' . $month . ' AND status = 1 and lost = 0')->result_array();
        $month_dates = [];
        $data        = [];
        for ($d = 1; $d <= 31; $d++) {
    ...
    public function get_stats_chart_data($label, $where, $dataset_options, $year)
    {
        ...
        foreach ($categories as $category) {
            ...
            array_push($chart['datasets'][0]['data'], total_rows( 'expenses', $_where));
        }
    ...
    public function get_expenses_vs_income_report($year = '')
    {
        ...
        for ($m = 1; $m <= 12; $m++) {
            ...
            $this->db->select('id')->from( 'expenses')->where('MONTH(date)', $m)->where('YEAR(date)', $year);
            ...
            $this->db->where('MONTH(' .  'invoicepaymentrecords.date)', $m);
            $this->db->where('YEAR(' .  'invoicepaymentrecords.date)', $year);
    ...
    public function leads_this_week_report()
    {
        ...
        $weekly = $this->db->get( 'leads')->result_array();
        ...
        foreach ($staff as $member) {
            ...
            if (!isset($to_date) && !isset($from_date)) {
                $this->db->where('CASE WHEN assigned=0 THEN addedfrom=' . $member['staffid'] . ' ELSE assigned=' . $member['staffid'] . ' END
                    AND status=1', '', false);
                $total_rows_converted = $this->db->count_all_results( 'leads');

                $total_rows_created = total_rows( 'leads', [
                    'addedfrom' => $member['staffid'],
                ]);

                $this->db->where('CASE WHEN assigned=0 THEN addedfrom=' . $member['staffid'] . ' ELSE assigned=' . $member['staffid'] . ' END
                    AND lost=1', '', false);
                $total_rows_lost = $this->db->count_all_results( 'leads');
            } else {
                $sql                  = 'SELECT COUNT(' .  'leads.id) as total FROM ' .  "leads WHERE DATE(last_status_change) BETWEEN '" . $this->db->escape_str($from_date) . "' AND '" . $this->db->escape_str($to_date) . "' AND status = 1 AND CASE WHEN assigned=0 THEN addedfrom=" . $member['staffid'] . ' ELSE assigned=' . $member['staffid'] . ' END';
                $total_rows_converted = $this->db->query($sql)->row()->total;

                $sql                = 'SELECT COUNT(' .  'leads.id) as total FROM ' .  "leads WHERE DATE(dateadded) BETWEEN '" . $this->db->escape_str($from_date) . "' AND '" . $this->db->escape_str($to_date) . "' AND addedfrom=" . $member['staffid'] . '';
                $total_rows_created = $this->db->query($sql)->row()->total;

                $sql = 'SELECT COUNT(' .  'leads.id) as total FROM ' .  "leads WHERE DATE(last_status_change) BETWEEN '" . $this->db->escape_str($from_date) . "' AND '" . $this->db->escape_str($to_date) . "' AND lost = 1 AND CASE WHEN assigned=0 THEN addedfrom=" . $member['staffid'] . ' ELSE assigned=' . $member['staffid'] . ' END';
    ...
    public function leads_sources_report()
    {
        ...
        foreach ($sources as $source) {
            ...
            array_push($chart['datasets'][0]['data'], total_rows( 'leads', [
                'source' => $source['id'],
                'status' => 1,
                'lost'   => 0,
            ]));
    ...
    public function report_by_customer_groups()
    {
        ...
        if ($months_report != '') {
            ...
            if (is_numeric($months_report)) {
                ...
                $custom_date_select = '(' .  'invoicepaymentrecords.date BETWEEN "' . $beginMonth . '" AND "' . $endMonth . '")';
            } elseif ($months_report == 'this_month') {
                $custom_date_select = '(' .  'invoicepaymentrecords.date BETWEEN "' . date('Y-m-01') . '" AND "' . date('Y-m-t') . '")';
            } elseif ($months_report == 'this_year') {
                $custom_date_select = '(' .  'invoicepaymentrecords.date BETWEEN "' .
                ...
                if ($from_date == $to_date) {
                    $custom_date_select =  'invoicepaymentrecords.date ="' . $from_date . '"';
                } else {
                    $custom_date_select = '(' .  'invoicepaymentrecords.date BETWEEN "' . $from_date . '" AND "' . $to_date . '")';
                }
            }
            $this->db->where($custom_date_select);
        }
        $this->db->select('amount,' .  'invoicepaymentrecords.date,' .  'invoices.clientid,(SELECT GROUP_CONCAT(name) FROM ' .  'customers_groups LEFT JOIN ' .  'customer_groups ON ' .  'customer_groups.groupid = ' .  'customers_groups.id WHERE customer_id = ' .  'invoices.clientid) as customerGroups');
        $this->db->from( 'invoicepaymentrecords');
        $this->db->join( 'invoices',  'invoices.id = ' .  'invoicepaymentrecords.invoiceid');
        $this->db->where( 'invoices.clientid IN (select customer_id FROM ' .  'customer_groups)');
        $this->db->where( 'invoices.status !=', 5);
    ...
    public function report_by_payment_modes()
    {
        ...
        $colors = get_system_favourite_colors();
        $this->db->select('amount,' .  'invoicepaymentrecords.date');
        $this->db->from( 'invoicepaymentrecords');
        $this->db->where('YEAR(' .  'invoicepaymentrecords.date)', $year);
        $this->db->join( 'invoices', '' .  'invoices.id = ' .  'invoicepaymentrecords.invoiceid');
        $by_currency = $this->input->post('report_currency');
        ...
        foreach ($modes as $mode) {
            if (total_rows( 'invoicepaymentrecords', [
                'paymentmode' => $mode['id'],
            ]) == 0) {
                continue;
            }
            ...
            $this->db->from( 'invoicepaymentrecords');
            $this->db->where('YEAR(' .  'invoicepaymentrecords.date)', $year);
            $this->db->where( 'invoicepaymentrecords.paymentmode', $mode['id']);
            $this->db->join( 'invoices', '' .  'invoices.id = ' .  'invoicepaymentrecords.invoiceid');
            $by_currency = $this->input->post('report_currency');
            if ($by_currency) {
                $this->db->where('currency', $by_currency);
    ...
    {
        return $this->db->query('SELECT DISTINCT(YEAR(date)) as year FROM ' .  'invoicepaymentrecords')->result_array();
    }

    public function get_distinct_customer_invoices_years()
    {
        return $this->db->query('SELECT DISTINCT(YEAR(date)) as year FROM ' .  'invoices WHERE clientid=' . get_client_user_id())->result_array();
    }
    ...
    protected function calculate_refunded_amount($year, $month, $currency)
    {
        ...
        if ($currency) {
            $sql .= ' AND credit_note_id IN (SELECT id FROM ' .  'creditnotes WHERE currency=' . $currency . ')';
        }
