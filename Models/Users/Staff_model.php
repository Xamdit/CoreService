...
class Staff_model extends App_Model
{
    public function delete($id, $transfer_data_to)
    {
        ...
        $this->db->update( 'newsfeed_post_comments', [
            'userid' => $transfer_data_to,
        ]);

        $this->db->where('creator', $id);
        $this->db->update( 'newsfeed_posts', [
            'creator' => $transfer_data_to,
        ]);

        $this->db->where('staff_id', $id);
        $this->db->update( 'projectdiscussions', [
        ...
        $this->db->where('staffid', $id);
        $this->db->update( 'proposal_comments', [
            'staffid' => $transfer_data_to,
        ]);
        ...
        $this->db->update( 'contract_renewals', [
        ...
        $this->db->where('assigned', $id);
        $this->db->update( 'estimate_requests', [
            'assigned' => $transfer_data_to,
        ]);
        ...
        $web_to_lead = $this->db->get( 'web_to_lead')->result_array();
        ...
        $this->db->where('notify_type', 'specific_staff');
        $estimate_requests = $this->db->get( 'estimate_request_forms')->result_array();
        ...
        $this->db->where('id', 1);
        $leads_email_integration = $this->db->get( 'leads_email_integration')->row();
        ...
        $this->db->where('userid', $id);
        $this->db->delete( 'newsfeed_comment_likes');

        $this->db->where('userid', $id);
        $this->db->delete( 'newsfeed_post_likes');
        ...
        $this->db->where('staffid', $id);
        $this->db->delete( 'staff_departments');
    ...
    public function get($id = '', $where = [])
    {
        ...
        if (is_staff_logged_in() && $id != '' && $id == get_staff_user_id()) {
            $select_str .= ',(SELECT COUNT(*) FROM ' .  'notifications WHERE touserid=' . get_staff_user_id() . ' and isread=0) as total_unread_notifications, (SELECT COUNT(*) FROM ' .  'todos WHERE finished=0 AND staffid=' . get_staff_user_id() . ') as total_unfinished_todos';
        }
    ...
    public function add($data)
    {
        ...
        if ($staffid) {
            ...
            foreach ($announcements as $announcement) {
                $this->db->insert( 'dismissed_announcements', [
        ...
        if (sizeof($staff_departments) > 0) {
            ...
            if (isset($departments)) {
                foreach ($departments as $department) {
                    ...
                    $_exists = $this->db->get( 'staff_departments')->row();
                    if (!$_exists) {
                        $this->db->insert( 'staff_departments', [
                            'staffid'      => $id,
                            'departmentid' => $department,
    ...
    public function get_logged_time_data($id = '', $filter_data = [])
    {
        ...
        $this->db->select('task_id,start_time,end_time,staff_id,' .  'taskstimers.hourly_rate,name,' .  'taskstimers.id,rel_id,rel_type, billed');
        $this->db->where('staff_id', $id);
        $this->db->join( 'tasks',  'tasks.id = ' .  'taskstimers.task_id', 'left');
        $timers           = $this->db->get( 'taskstimers')->result_array();
