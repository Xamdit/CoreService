...
class Clients_model extends App_Model
{
    ...
    public function get($id = '', $where = [])
    {
        ...
        if (is_numeric($id)) {
            $this->db->where( 'clients.userid', $id);
            $client = $this->db->get( 'clients')->row();

            if ($client && db.get_option('company_requires_vat_number_field') == 0) {
                $client->vat = null;
            }
    ...
    public function add($data, $withContact = false)
    {
        ...
        $this->db->insert( 'clients', array_merge($data, [
            ...
            if (sizeof($customer_permissions) > 0) {
                foreach ($customer_permissions as $customer_permission) {
                    if (!in_array($customer_permission['permission_id'], $permissions)) {
                        ...
                        $this->db->delete( 'contact_permissions');
                        if ($this->db->affected_rows() > 0) {
                            $affectedRows++;
                        }
                    }
                ...
                foreach ($permissions as $permission) {
                    ...
                    $_exists = $this->db->get( 'contact_permissions')->row();
                    if (!$_exists) {
                        $this->db->insert( 'contact_permissions', [
                            'userid'        => $id,
    ...
    public function add_contact($data, $customer_id, $not_manual_request = false)
    {
        ...
        if ($contact_id) {
            ...
            if (!isset($permissions) && $not_manual_request == false) {
                ...
                $default_permissions = @unserialize(db.get_option('default_contact_permissions'));
            ...
            foreach ($permissions as $permission) {
                ...
                if ($not_manual_request == true) {
                    if ($permission == 6) {
                        $this->db->where('id', $contact_id);
                        $this->db->update( 'contacts', ['project_emails' => 1, 'task_emails' => 1]);
                    } elseif ($permission == 3) {
                        $this->db->where('id', $contact_id);
                        $this->db->update( 'contacts', ['contract_emails' => 1]);
                    } elseif ($permission == 2) {
                        $this->db->where('id', $contact_id);
                        $this->db->update( 'contacts', ['estimate_emails' => 1]);
                    } elseif ($permission == 1) {
                        $this->db->where('id', $contact_id);
                        $this->db->update( 'contacts', ['invoice_emails' => 1, 'credit_note_emails' => 1]);
                    } elseif ($permission == 5) {
                        $this->db->where('id', $contact_id);
                        $this->db->update( 'contacts', ['ticket_emails' => 1]);
                    }
                }
            }
    ...
    public function get_admins($id)
    {
        ...
        return $this->db->get( 'customer_admins')->result_array();
    }
    ...
    {
        return $this->db->query('SELECT DISTINCT(staff_id) FROM ' .  'customer_admins')->result_array();
    }
    ...
    public function assign_admins($data, $id)
    {
        ...
        if (count($data) == 0) {
            $this->db->where('customer_id', $id);
            $this->db->delete( 'customer_admins');
            if ($this->db->affected_rows() > 0) {
                $affectedRows++;
            }
        } else {
            ...
            foreach ($data['customer_admins'] as $n_admin_id) {
                if (total_rows( 'customer_admins', [
                    'customer_id' => $id,
                    'staff_id' => $n_admin_id,
                ]) == 0) {
                    $this->db->insert( 'customer_admins', [
                        'customer_id'   => $id,
                        'staff_id'      => $n_admin_id,
    ...
    public function delete($id)
    {
        ...
        if (!is_gdpr() && is_reference_in_table('clientid',  'invoices', $id)) {
            return [
                'referenced' => true,
            ];
        }

        if (!is_gdpr() && is_reference_in_table('clientid',  'estimates', $id)) {
            return [
                'referenced' => true,
            ];
        }

        if (!is_gdpr() && is_reference_in_table('clientid',  'creditnotes', $id)) {
            return [
                'referenced' => true,
            ];
        }
        ...
        if ($this->db->affected_rows() > 0) {
            ...
            if (is_gdpr() && db.get_option('gdpr_on_forgotten_remove_invoices_credit_notes') == '1') {
                ...
                $this->db->where('clientid', $id);
                $credit_notes = $this->db->get( 'creditnotes')->result_array();
                foreach ($credit_notes as $credit_note) {
                    $this->credit_notes_model->delete($credit_note['id'], true);
                }
            } elseif (is_gdpr()) {
                $this->db->where('clientid', $id);
                $this->db->update( 'invoices', ['deleted_customer_name' => $company]);

                $this->db->where('clientid', $id);
                $this->db->update( 'creditnotes', ['deleted_customer_name' => $company]);
            }
            ...
            if (is_gdpr() && db.get_option('gdpr_on_forgotten_remove_estimates') == '1') {
                ...
                $this->db->where('clientid', $id);
                $this->db->update( 'estimates', ['deleted_customer_name' => $company]);
            }
            ...
            $this->db->where('clientid', $id);
            $subscriptions = $this->db->get( 'subscriptions')->result_array();
            ...
            $this->db->where('client_id', $id);
            $this->db->update( 'leads', ['client_id' => 0]);
    ...
    public function delete_contact($id)
    {
        ...
        if ($this->db->affected_rows() > 0) {
            ...
            $this->db->delete( 'consents');

            $this->db->where('contact_id', $id);
            $this->db->delete( 'shared_customer_files');

            $this->db->where('userid', $id);
            $this->db->where('staff', 0);
            $this->db->delete( 'dismissed_announcements');
            ...
            $this->db->where('contact_id', $id);
            $filesUploadedFromContactToTasks = $this->db->get( 'files')->result_array();
            ...
            $this->db->where('contact_id', $id);
            $tasksComments = $this->db->get( 'task_comments')->result_array();
            ...
            $this->db->where('contact_id', $id);
            $files = $this->db->get( 'project_files')->result_array();
            ...
            $this->db->where('contact_id', $id);
            $discussionsComments = $this->db->get( 'projectdiscussioncomments')->result_array();
            ...
            if (is_gdpr()) {
                ...
                $this->db->delete( 'listemails');

                if (!empty($result->last_ip)) {
                    $this->db->where('ip', $result->last_ip);
                    $this->db->delete( 'knowedge_base_article_feedback');
                }

                $this->db->where('email', $result->email);
                $this->db->delete( 'tickets_pipe_log');

                $this->db->where('email', $result->email);
                $this->db->delete( 'tracked_mails');
    ...
    public function change_contact_password($id, $oldPassword, $newPassword)
    {
        ...
        $this->db->where('id', $id);
        $client = $this->db->get( 'contacts')->row();
    ...
    */
    ...
    public function require_confirmation($client_id)
    {
        ...
        $this->db->update( 'clients', ['active' => 0, 'registration_confirmed' => 0]);

        $this->db->where('id', $contact_id);
        $this->db->update( 'contacts', ['active' => 0]);
    ...
    public function confirm_registration($client_id)
    {
        $contact_id = get_primary_contact_user_id($client_id);
        $this->db->where('userid', $client_id);
        $this->db->update( 'clients', ['active' => 1, 'registration_confirmed' => 1]);

        $this->db->where('id', $contact_id);
        $this->db->update( 'contacts', ['active' => 1]);
    ...
    public function send_verification_email($id)
    {
        ...
        if ($success) {
            $this->db->where('id', $id);
            $this->db->update( 'contacts', ['email_verification_sent_at' => date('Y-m-d H:i:s')]);
        }
    ...
    {
        return $this->db->query('SELECT DISTINCT(country_id), short_name FROM ' .  'clients JOIN ' .  'countries ON ' .  'countries.country_id=' .  'clients.country')->result_array();
    }
    ...
    public function get_staff_members_that_can_access_customer($id)
    {
        ...
        return $this->db->query('SELECT * FROM ' .  'staff
            WHERE (
                    admin=1
                    OR staffid IN (SELECT staff_id FROM ' .  "customer_admins WHERE customer_id='.$id.')
                    OR staffid IN(SELECT staff_id FROM " .  'staff_permissions WHERE feature = "customers" AND capability="view")
                )
