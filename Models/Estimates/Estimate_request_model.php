...
class Estimate_request_model extends App_Model
{
    ...
    public function update_request_assigned($data)
    {
        ...
        $_old_assigned = $this->db->get( 'estimate_requests')->row();
        ...
        $this->db->select('assigned');
        $this->db->where('id', $data['requestid']);
        $_current_assigned = $this->db->get( 'estimate_requests')->row();
    ...
    public function update_request_status($data)
    {
        ...
        $this->db->where('id', $data['requestid']);
        $_old = $this->db->get( 'estimate_requests')->row();
    ...
    {
        $this->db->select('*,' .  'estimate_requests.id,' .  'estimate_request_status.name as status_name');
        $this->db->join( 'estimate_request_status',  'estimate_request_status.id=' .  'estimate_requests.status', 'left');

        $this->db->where($where);
        if (is_numeric($id)) {
            $this->db->where( 'estimate_requests.id', $id);
            $request = $this->db->get( 'estimate_requests')->row();
            if ($request) {
        ...
        return $this->db->get( 'estimate_requests')->result_array();
    }

    public function get_forms()
    {
        return $this->db->get( 'estimate_request_forms')->result_array();
    }
    ...
    public function get_form($where)
    {
        ...
        return $this->db->get( 'estimate_request_forms')->row();
    }
    ...
    public function add_form($data)
    {
        ...
        $this->db->insert( 'estimate_request_forms', $data);
    ...
    public function update_form($id, $data)
    {
        ...
        $data['success_submit_msg'] = nl2br($data['success_submit_msg']);
        $this->db->where('id', $id);
        $this->db->update( 'estimate_request_forms', $data);
    ...
    {
        $this->db->where('id', $id);
        $this->db->delete( 'estimate_request_forms');
    ...
    public function delete($id)
    {
        ...
        $this->db->where('id', $id);
        $this->db->delete( 'estimate_requests');
        if ($this->db->affected_rows() > 0) {
    ...
    public function get_status($id = '', $where = [])
    {
        ...
        if (is_numeric($id)) {
            ...
            return $this->db->get( 'estimate_request_status')->row();
        }
        ...
        if (!$statuses) {
            ...
            $statuses = $this->db->get( 'estimate_request_status')->result_array();
    ...
    public function add_status($data)
    {
        ...
        if (!isset($data['statusorder'])) {
            $data['statusorder'] = total_rows( 'estimate_request_status') + 1;
        }

        $this->db->insert( 'estimate_request_status', $data);
        $insert_id = $this->db->insert_id();
        if ($insert_id) {
    ...
    {
        $this->db->where('id', $id);
        $this->db->update( 'estimate_request_status', $data);
        if ($this->db->affected_rows() > 0) {
    ...
    public function delete_status($id)
    {
        ...
        if (is_reference_in_table('status',  'estimate_requests', $id)) {
            return [
                'referenced' => true,
            ];
        }
        ...
        $this->db->where('id', $id);
        $this->db->delete( 'estimate_request_status');
        if ($this->db->affected_rows() > 0) {
