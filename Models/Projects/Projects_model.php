...
class Projects_model extends App_Model
{
    ...
    {
        return $this->db->query('SELECT DISTINCT staff_id FROM ' .  'taskstimers LEFT JOIN ' .  'tasks ON ' .  'tasks.id = ' .  'taskstimers.task_id WHERE rel_type="project" AND rel_id=' . $this->db->escape_str($project_id))->result_array();
    }

    public function get_distinct_projects_members()
    {
        return $this->db->query('SELECT staff_id, firstname, lastname FROM ' .  'project_members JOIN ' .  'staff ON ' .  'staff.staffid=' .  'project_members.staff_id GROUP by staff_id order by firstname ASC')->result_array();
    }
    ...
    public function timers_started_for_project($project_id, $where = [], $task_timers_where = [])
    {
        ...
        $this->db->where( 'tasks.rel_id', $project_id);
        $this->db->where( 'tasks.rel_type', 'project');
        $this->db->join( 'tasks',  'tasks.id=' .  'taskstimers.task_id');
        $total = $this->db->count_all_results( 'taskstimers');
    ...
    {
        if (total_rows( 'pinned_projects', [
            'staff_id' => get_staff_user_id(),
            'project_id' => $id,
        ]) == 0) {
            $this->db->insert( 'pinned_projects', [
                'staff_id'   => get_staff_user_id(),
                'project_id' => $id,
            ]);
        ...
        $this->db->delete( 'pinned_projects');
    ...
    {
        $total_project_tasks = total_rows( 'tasks', [
            'rel_type' => 'project',
            'rel_id'   => $id,
        ]);
        $total_finished_tasks = total_rows( 'tasks', [
            'rel_type' => 'project',
            'rel_id'   => $id,
            'status'   => 5,
        ]);
    ...
    public function get_last_project_settings()
    {
        ...
        $this->db->limit(1);
        $last_project = $this->db->get( 'projects')->row();
        if ($last_project) {
                    ...
                    foreach ($tabs_flatten as $tab) {
                        if (!in_array($tab, $settings_available_features)) {
                            if ($available_features_index) {
                                ...
                                $this->db->update( 'project_settings', ['value' => serialize($tmp)]);
                            }
                        }
                    }
                }
    ...
    public function get_tasks($id, $where = [], $apply_restrictions = false, $count = false, $callback = null)
    {
        ...
        $show_all_tasks_for_project_member = db.get_option('show_all_tasks_for_project_member');

        $select = implode(', ', prefixed_table_fields_array( 'tasks')) . ',' .  'milestones.name as milestone_name,
        (SELECT SUM(CASE
            WHEN end_time is NULL THEN ' . time() . '-start_time
            ELSE end_time-start_time
            END) FROM ' .  'taskstimers WHERE task_id=' .  'tasks.id) as total_logged_time,
           ' . get_sql_select_task_assignees_ids() . ' as assignees_ids
        ';

        if (!is_client_logged_in() && is_staff_logged_in()) {
            $select .= ',(SELECT staffid FROM ' .  'task_assigned WHERE taskid=' .  'tasks.id AND staffid=' . get_staff_user_id() . ') as current_user_is_assigned';
        }
        ...
        $this->db->join( 'milestones',  'milestones.id = ' .  'tasks.milestone', 'left');
        $this->db->where('rel_id', $id);
        $this->db->where('rel_type', 'project');
        if ($apply_restrictions == true) {
            if (!is_client_logged_in() && !$has_permission && $show_all_tasks_for_project_member == 0) {
                $this->db->where('(
                    ' .  'tasks.id IN (SELECT taskid FROM ' .  'task_assigned WHERE staffid=' . get_staff_user_id() . ')
                    OR ' .  'tasks.id IN(SELECT taskid FROM ' .  'task_followers WHERE staffid=' . get_staff_user_id() . ')
                    OR is_public = 1
        ...
        if (isset($where[ 'milestones.hide_from_customer'])) {
            $this->db->group_start();
            $this->db->where( 'milestones.hide_from_customer', $where[ 'milestones.hide_from_customer']);
            $this->db->or_where( 'tasks.milestone', 0);
            $this->db->group_end();
            unset($where[ 'milestones.hide_from_customer']);
        }
        ...
        if ($count == false) {
            ...
            $tasks = $this->db->count_all_results( 'tasks');
        }
    ...
    public function do_milestones_kanban_query($milestone_id, $project_id, $page = 1, $where = [], $count = false)
    {
        ...
        $limit              = db.get_option('tasks_kanban_limit');
    ...
    public function get_file($id, $project_id = false)
    {
        ...
        $this->db->where('id', $id);
        $file = $this->db->get( 'project_files')->row();
    ...
    {
        $this->db->where('id', $id);
        $this->db->update( 'project_activity', [
            'visible_to_customer' => $visible,
        ]);
    }
    ...
    public function total_logged_time($id)
    {
        $q = $this->db->query('
            ...
            FROM ' .  'taskstimers
            WHERE task_id IN (SELECT id FROM ' .  'tasks WHERE rel_type="project" AND rel_id=' . $this->db->escape_str($id) . ')')
            ->row();
    ...
    {
        $this->db->select('*, (SELECT COUNT(id) FROM ' .  'tasks WHERE rel_type="project" AND rel_id=' . $this->db->escape_str($project_id) . ' and milestone=' .  'milestones.id) as total_tasks, (SELECT COUNT(id) FROM ' .  'tasks WHERE rel_type="project" AND rel_id=' . $this->db->escape_str($project_id) . ' and milestone=' .  'milestones.id AND status=5) as total_finished_tasks');
        $this->db->where('project_id', $project_id);
        $this->db->order_by('milestone_order', 'ASC');
        $this->db->where($where);
        $milestones = $this->db->get( 'milestones')->result_array();
        $i          = 0;
    ...
    public function add_milestone($data)
    {
        ...
        if ($insert_id) {
            ...
            $milestone = $this->db->get( 'milestones')->row();
    ...
    public function update($data, $id)
    {
        ...
        $this->db->where('id', $id);
        $old_status = $this->db->get( 'projects')->row()->status;
        ...
        if (!isset($data['settings'])) {
            $this->db->where('project_id', $id);
            $this->db->update( 'project_settings', [
                'value' => 0,
            ]);
            if ($this->db->affected_rows() > 0) {
                $affectedRows++;
        ...
        if ($affectedRows > 0) {
            ...
            if ($original_project->status != $data['status']) {
                ...
                if ($data['status'] == 4) {
                    ...
                    $this->db->where('id', $id);
                    $this->db->update( 'projects', ['date_finished' => date('Y-m-d H:i:s')]);
                } else {
    ...
    public function mark_as($data)
    {
        ...
        if ($this->db->affected_rows() > 0) {
            ...
            if ($data['status_id'] == 4) {
                ...
                if ($old_status == 4) {
                    $this->db->where('id', $data['project_id']);
                    $this->db->update( 'projects', ['date_finished' => null]);
                }
            }
        ...
        if (sizeof($project_members_in) > 0) {
            ...
            if (isset($project_members)) {
                ...
                foreach ($project_members as $staff_id) {
                    ...
                    $_exists = $this->db->get( 'project_members')->row();
                    if (!$_exists) {
                        if (empty($staff_id)) {
                            continue;
                        }
    ...
    public function is_member($project_id, $staff_id = '')
    {
        ...
        $member = total_rows( 'project_members', [
            'staff_id'   => $staff_id,
    ...
    public function get_project_settings($project_id)
    {
        ...
        return $this->db->get( 'project_settings')->result_array();
    }
    ...
    public function get_project_members($id, $with_name = false)
    {
        ...
        $this->db->join( 'staff',  'staff.staffid=' .  'project_members.staff_id');
        $this->db->where('project_id', $id);

        return $this->db->get( 'project_members')->result_array();
    }
    ...
    public function remove_team_member($project_id, $staff_id)
    {
        ...
        if ($this->db->affected_rows() > 0) {
            ...
            $this->db->where('taskid IN (SELECT id FROM ' .  'tasks WHERE rel_type="project" AND rel_id="' . $this->db->escape_str($project_id) . '")');
    ...
    public function get_timesheets($project_id, $tasks_ids = [])
    {
        ...
        if (count($tasks_ids) > 0) {
            ...
            $timesheets = $this->db->get( 'taskstimers')->result_array();
            $i          = 0;
    ...
    public function get_discussion($id, $project_id = '')
    {
        ...
        if (is_client_logged_in()) {
            ...
            $this->db->where('project_id IN (SELECT id FROM ' .  'projects WHERE clientid=' . get_client_user_id() . ')');
        }
        $discussion = $this->db->get( 'projectdiscussions')->row();
        if ($discussion) {
            return $discussion;
        }
    ...
    {
        $this->db->where('id', $id);
        $comment = $this->db->get( 'projectdiscussioncomments')->row();
    ...
    public function get_discussion_comments($id, $type)
    {
        ...
        $comments             = $this->db->get( 'projectdiscussioncomments')->result_array();
        $i                    = 0;
        $allCommentsIDS       = [];
    ...
    public function get_discussions($project_id)
    {
        ...
        $discussions = $this->db->get( 'projectdiscussions')->result_array();
        $i           = 0;
        foreach ($discussions as $discussion) {
            $discussions[$i]['total_comments'] = total_rows( 'projectdiscussioncomments', [
    ...
    public function add_discussion_comment($data, $discussion_id, $type)
    {
        ...
        $this->db->insert( 'projectdiscussioncomments', $_data);
        $insert_id = $this->db->insert_id();
        if ($insert_id) {
            if ($type == 'regular') {
                $discussion = $this->get_discussion($discussion_id);
    ...
    public function update_discussion_comment($data)
    {
        ...
        $this->db->where('id', $data['id']);
        $this->db->update( 'projectdiscussioncomments', [
    ...
    public function delete_discussion_comment($id, $logActivity = true)
    {
        ...
        $this->db->where('id', $id);
        $this->db->delete( 'projectdiscussioncomments');
        if ($this->db->affected_rows() > 0) {
    ...
    public function add_discussion($data)
    {
        ...
        $data['description'] = nl2br($data['description']);
        $this->db->insert( 'projectdiscussions', $data);
        $insert_id = $this->db->insert_id();
        if ($insert_id) {
    ...
    public function edit_discussion($data, $id)
    {
        ...
        $data['description'] = nl2br($data['description']);
        $this->db->update( 'projectdiscussions', $data);
        if ($this->db->affected_rows() > 0) {
    ...
    public function delete_discussion($id, $logActivity = true)
    {
        ...
        $this->db->where('id', $id);
        $this->db->delete( 'projectdiscussions');
        if ($this->db->affected_rows() > 0) {
            if ($logActivity) {
    ...
    public function copy($project_id, $data)
    {
        ...
        $this->db->insert( 'projects', $_new_data);
    ...
    public function get_staff_notes($project_id)
    {
        $this->db->where('project_id', $project_id);
        $this->db->where('staff_id', get_staff_user_id());
        $notes = $this->db->get( 'project_notes')->row();
        if ($notes) {
    ...
    public function delete($project_id)
    {
        ...
        if ($this->db->affected_rows() > 0) {
            ...
            $this->db->where('project_id', $project_id);
            $this->db->delete( 'project_activity');
    ...
    public function get_activity($id = '', $limit = '', $only_project_members_activity = false)
    {
        if (!is_client_logged_in()) {
            ...
            if (!$has_permission) {
                $this->db->where('project_id IN (SELECT project_id FROM ' .  'project_members WHERE staff_id=' . get_staff_user_id() . ')');
            }
        }
        if (is_client_logged_in()) {
            $this->db->where('visible_to_customer', 1);
        ...
        $activities = $this->db->get( 'project_activity')->result_array();
        $i          = 0;
    ...
    public function convert_estimate_items_to_tasks($project_id, $items, $assignees, $project_data, $project_settings)
    {
        ...
        foreach ($items as $index => $itemId) {
            ...
            $_item = $this->db->get( 'itemable')->row();
