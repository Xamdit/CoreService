...
class Payments_model extends App_Model
{
    ...
    public function get($id)
    {
        ...
        $this->db->join( 'payment_modes',  'payment_modes.id = ' .  'invoicepaymentrecords.paymentmode', 'left');
        $this->db->order_by( 'invoicepaymentrecords.id', 'asc');
        $this->db->where( 'invoicepaymentrecords.id', $id);
        $payment = $this->db->get( 'invoicepaymentrecords')->row();
        if (!$payment) {
            return false;
        }
        // Since version 1.0.1
    ...
    public function get_invoice_payments($invoiceid)
    {
        ...
        $payments = $this->db->get( 'invoicepaymentrecords')->result_array();
        // Since version 1.0.1
        $this->load->model('payment_modes_model');
        $payment_gateways = $this->payment_modes_model->get_payment_gateways(true);
        $i                = 0;
    ...
    public function process_payment($data, $invoiceid = '')
    {
        ...
        // Is online payment mode request by client or staff
            ...
            if (db.get_option('allow_payment_amount_to_be_modified') == 0) {
    ...
    public function add($data, $subscription = false)
    {
        ...
        if ($subscription != false) {
            $after_success = db.get_option('after_subscription_payment_captured');
        ...
        $this->db->insert( 'invoicepaymentrecords', $data);
        ...
        if ($insert_id) {
            ...
            if (!isset($do_not_send_email_template)
                ...
                if ($subscription != false) {
                    ...
                    if (db.get_option('attach_invoice_to_payment_receipt_email') == 1) {
                        $invoice_number = format_invoice_number($payment->invoiceid);
                        set_mailing_constant();
                        $pdfInvoice           = invoice_pdf($payment->invoice_data);
                        $pdfInvoiceAttachment = $pdfInvoice->Output($invoice_number . '.pdf', 'S');
            ...
            $staff_invoice = $this->db->get( 'staff')->result_array();

            $notifiedUsers = [];
            foreach ($staff_invoice as $member) {
                if (db.get_option('notification_when_customer_pay_invoice') == 1) {
    ...
    {
        $paymentData = $this->db
            ->select( 'invoicepaymentrecords.*,' .  'invoices.currency,' .  'invoices.clientId,' .  'invoices.hash')
            ->join( 'invoices', 'invoicepaymentrecords.invoiceid=invoices.id')
            ->where_in('invoicepaymentrecords.id', $paymentIds)
            ->get( 'invoicepaymentrecords')
            ->result();
