...
class Dashboard_model extends App_Model
{
    ...
    public function get_weekly_payments_statistics($currency)
    {
        ...
        $this->db->select( 'invoicepaymentrecords.id, amount,' .  'invoicepaymentrecords.date');
        ...
        if (!$has_permission_payments_view) {
            $this->db->where('invoiceid IN (SELECT id FROM ' .  'invoices WHERE addedfrom=' . get_staff_user_id() . ' and addedfrom IN (SELECT staff_id FROM ' .  'staff_permissions WHERE feature="invoices" AND capability="view_own"))');
        }
    ...
    public function get_monthly_payments_statistics($currency)
    {
        $all_payments                 = [];
        $has_permission_payments_view = has_permission('payments', '', 'view');
        $this->db->select('SUM(amount) as total, MONTH(' .  'invoicepaymentrecords.date) as month');
        $this->db->from( 'invoicepaymentrecords');
        $this->db->join( 'invoices', '' .  'invoices.id = ' .  'invoicepaymentrecords.invoiceid');
        $this->db->where('YEAR(' .  'invoicepaymentrecords.date) = YEAR(CURRENT_DATE)');
        $this->db->where('' .  'invoices.status !=', 5);
    ...
    public function projects_status_stats()
    {
        ...
        foreach ($statuses as $status) {
            ...
            if (!$has_permission) {
                $sql .= ' AND id IN (SELECT project_id FROM ' .  'project_members WHERE staff_id=' . get_staff_user_id() . ')';
            }
            $sql .= ' UNION ALL ';
            $sql = trim($sql);
        }
    ...
    public function tickets_awaiting_reply_by_department()
    {
        ...
        foreach ($departments as $department) {
            if (!is_admin()) {
                if (db.get_option('staff_access_only_assigned_departments') == 1) {
                    ...
                    if (count($departments_ids) > 0) {
                        $this->db->where('department IN (SELECT departmentid FROM ' .  'staff_departments WHERE departmentid IN (' . implode(',', $departments_ids) . ') AND staffid="' . get_staff_user_id() . '")');
                    }
                }
            }
            ...
            $this->db->where( 'tickets.merged_ticket_id IS NULL', null, false);
