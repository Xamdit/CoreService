...
class Proposals_model extends App_Model
{
    ...
    {
        return $this->db->query('SELECT DISTINCT(assigned) as sale_agent FROM ' .  'proposals WHERE assigned != 0')->result_array();
    }

    public function get_proposals_years()
    {
        return $this->db->query('SELECT DISTINCT(YEAR(date)) as year FROM ' .  'proposals')->result_array();
    }
    ...
    public function add($data)
    {
        ...
        if ($insert_id) {
            ...
            update_sales_total_tax_column($insert_id, 'proposal',  'proposals');
    ...
    public function update($data, $id)
    {
        ...
        if ($affectedRows > 0) {
            update_sales_total_tax_column($id, 'proposal',  'proposals');
    ...
    public function get($id = '', $where = [], $for_editor = false)
    {
        ...
        $this->db->select('*,' .  'currencies.id as currencyid, ' .  'proposals.id as id, ' .  'currencies.name as currency_name');
        $this->db->from( 'proposals');
        $this->db->join( 'currencies',  'currencies.id = ' .  'proposals.currency', 'left');
    ...
    public function clear_signature($id)
    {
        ...
        if ($proposal) {
            $this->db->where('id', $id);
            $this->db->update( 'proposals', ['signature' => null]);
    ...
    public function add_comment($data, $client = false)
    {
        ...
        $this->db->insert( 'proposal_comments', $data);
        $insert_id = $this->db->insert_id();
        if ($insert_id) {
            ...
            if ($client == true) {
                ...
                $staff_proposal = $this->db->get( 'staff')->result_array();
                $notifiedUsers  = [];
    ...
    public function get_comments($id)
    {
        ...
        return $this->db->get( 'proposal_comments')->result_array();
    }
    ...
    public function get_comment($id)
    {
        ...
        return $this->db->get( 'proposal_comments')->row();
    }
    ...
    public function remove_comment($id)
    {
        ...
        $this->db->where('id', $id);
        $this->db->delete( 'proposal_comments');
        if ($this->db->affected_rows() > 0) {
    ...
    public function copy($id)
    {
        ...
        if ($insert_data['open_till'] && db.get_option('proposal_due_after') != 0) {
            $insert_data['open_till'] = _d(date('Y-m-d', strtotime('+' . db.get_option('proposal_due_after') . ' DAY', strtotime(date('Y-m-d')))));
    ...
    public function delete($id)
    {
        ...
        if ($this->db->affected_rows() > 0) {
            ...
            $this->db->where('relid IN (SELECT id from ' .  'itemable WHERE rel_type="proposal" AND rel_id="' . $this->db->escape_str($id) . '")');
    ...
    public function send_proposal_to_email($id, $attachpdf = true, $cc = '')
    {
        ...
        if (total_rows( 'proposals', ['id' => $id, 'status' => 6]) > 0) {
            $this->db->where('id', $id);
            $this->db->update( 'proposals', ['status' => 4]);
        }
