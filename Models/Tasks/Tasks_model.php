...
class Tasks_model extends App_Model
{
    ...
    {
        $this->db->where('id IN (SELECT taskid FROM ' .  'task_assigned WHERE staffid = ' . get_staff_user_id() . ')');
    ...
    public function get($id, $where = [])
    {
        ...
        if ($task) {
            ...
            $task->timesheets      = $this->get_timesheets($id);
    ...
    public function get_milestone($id)
    {
        ...
        return $this->db->get( 'milestones')->row();
    }
    ...
    {
        return $this->db->query('SELECT DISTINCT(YEAR(' . $this->db->escape_str($get_from) . ')) as year FROM ' .  'tasks WHERE ' . $this->db->escape_str($get_from) . ' IS NOT NULL ORDER BY year DESC')->result_array();
    }

    public function is_task_billed($id)
    {
        return (total_rows( 'tasks', [
            'id'     => $id,
            'billed' => 1,
        ]) > 0 ? true : false);
    }
    ...
    public function copy_task_custom_fields($from_task, $to_task)
    {
        ...
        foreach ($custom_fields as $field) {
            ...
            if ($value != '') {
                $this->db->insert( 'customfieldsvalues', [
                    'relid'   => $to_task,
                    'fieldid' => $field['id'],
                    'fieldto' => 'tasks',
                    'value'   => $value,
    ...
    public function get_billable_tasks($customer_id = false, $project_id = '')
    {
        ...
        if ($customer_id != false && $project_id == '') {
            $this->db->where(
                '
                (
                (rel_id IN (SELECT id FROM ' .  'invoices WHERE clientid=' . $this->db->escape_str($customer_id) . ') AND rel_type="invoice")
                OR
                (rel_id IN (SELECT id FROM ' .  'estimates WHERE clientid=' . $this->db->escape_str($customer_id) . ') AND rel_type="estimate")
                OR
                (rel_id IN (SELECT id FROM ' .  'contracts WHERE client=' . $this->db->escape_str($customer_id) . ') AND rel_type="contract")
                OR
                ( rel_id IN (SELECT ticketid FROM ' .  'tickets WHERE userid=' . $this->db->escape_str($customer_id) . ') AND rel_type="ticket")
                OR
                (rel_id IN (SELECT id FROM ' .  'expenses WHERE clientid=' . $this->db->escape_str($customer_id) . ') AND rel_type="expense")
                OR
                (rel_id IN (SELECT id FROM ' .  'proposals WHERE rel_id=' . $this->db->escape_str($customer_id) . ' AND rel_type="customer") AND rel_type="proposal")
                OR
                (rel_id IN (SELECT userid FROM ' .  'clients WHERE userid=' . $this->db->escape_str($customer_id) . ') AND rel_type="customer")
                )'
            );
        }
    ...
    {
        $this->db->where($where);
        $this->db->where('(id IN (SELECT taskid FROM ' .  'task_assigned WHERE staffid=' . $this->db->escape_str($id) . '))');
    ...
    public function add($data, $clientRequest = false)
    {
        ...
        if ($clientRequest == false) {
            $defaultStatus = db.get_option('default_task_status');
        ...
        if ($insert_id) {
            ...
            if ($clientRequest == false) {
                ...
                if ($fromTicketId !== null) {
                    $ticket_attachments = $this->db->query('SELECT * FROM ' .  'ticket_attachments WHERE ticketid=' . $this->db->escape_str($fromTicketId) . ' OR (ticketid=' . $this->db->escape_str($fromTicketId) . ' AND replyid IN (SELECT id FROM ' .  'ticket_replies WHERE ticketid=' . $this->db->escape_str($fromTicketId) . '))')->result_array();
    ...
    public function get_checklist_item($id)
    {
        ...
        return $this->db->get( 'task_checklist_items')->row();
    }
    ...
    {
        $this->db->insert( 'tasks_checklist_templates', [
    ...
    {
        $this->db->where('id', $id);
        $this->db->delete( 'tasks_checklist_templates');
        if ($this->db->affected_rows() > 0) {
            return true;
        }
    ...
    public function get_checklist_templates()
    {
        ...
        return $this->db->get( 'tasks_checklist_templates')->result_array();
    }
    ...
    public function get_checklist_template($id)
    {
        ...
        return $this->db->get( 'tasks_checklist_templates')->row();
    }
    ...
    {
        $this->db->where('id', $id);
        $this->db->delete( 'task_checklist_items');
        if ($this->db->affected_rows() > 0) {
            return true;
        }
    ...
    public function update_checklist_order($data)
    {
        foreach ($data['order'] as $order) {
            ...
            $this->db->update( 'task_checklist_items', [
                'list_order' => $order[1],
            ]);
        }
    }
    ...
    public function get_task_creator_id($taskid)
    {
        ...
        return $this->db->get( 'tasks')->row()->addedfrom;
    }
    ...
    public function add_task_comment($data)
    {
        ...
        if ($insert_id) {
            ...
            if (preg_match_all($regex, $data['content'], $mentionedStaff, PREG_PATTERN_ORDER)) {
                ...
                $project_settings = $this->db->get( 'project_settings')->row();
    ...
    public function add_task_assignees($data, $cronOrIntegration = false, $clientRequest = false)
    {
        ...
        $this->db->insert( 'task_assigned', $assignData);
    ...
    {
        $this->db->select(implode(', ', prefixed_table_fields_array( 'files')) . ', ' .  'task_comments.id as comment_file_id');
        $this->db->where( 'files.rel_id', $taskid);
        $this->db->where( 'files.rel_type', 'task');

        if ((is_array($where) && count($where) > 0) || (is_string($where) && $where != '')) {
            $this->db->where($where);
        }

        $this->db->join( 'task_comments',  'task_comments.file_id = ' .  'files.id', 'left');
        $this->db->join( 'tasks',  'tasks.id = ' .  'files.rel_id');
        $this->db->order_by( 'files.dateadded', 'desc');
    ...
    public function remove_task_attachment($id)
    {
        ...
        // Get the attachment
        $this->db->where('id', $id);
        $attachment = $this->db->get( 'files')->row();
        ...
        if ($deleted) {
            if ($attachment->task_comment_id != 0) {
                $total_comment_files = total_rows( 'files', ['task_comment_id' => $attachment->task_comment_id]);
                if ($total_comment_files == 0) {
                    $this->db->where('id', $attachment->task_comment_id);
                    $comment = $this->db->get( 'task_comments')->row();
                    ...
                    if ($comment) {
                        ...
                        if (empty($comment->content) || $comment->content === '[task_attachment]') {
                            ...
                            $this->db->query('UPDATE ' .  "task_comments
            ...
            $comment_attachment = $this->db->get( 'task_comments')->row();
    ...
    {
        $this->db->select('id,' .  'task_followers.staffid as followerid, CONCAT(firstname, " ", lastname) as full_name');
        $this->db->from( 'task_followers');
        $this->db->join( 'staff',  'staff.staffid = ' .  'task_followers.staffid');
    ...
    {
        $this->db->select('id,' .  'task_assigned.staffid as assigneeid,assigned_from,firstname,lastname,CONCAT(firstname, " ", lastname) as full_name,is_assigned_from_contact');
        $this->db->from( 'task_assigned');
        $this->db->join( 'staff',  'staff.staffid = ' .  'task_assigned.staffid');
        $this->db->where('taskid', $id);
    ...
    public function get_task_comments($id)
    {
        ...
        $this->db->select('id,dateadded,content,' .  'staff.firstname,' .  'staff.lastname,' .  'task_comments.staffid,' .  'task_comments.contact_id as contact_id,file_id,CONCAT(firstname, " ", lastname) as staff_full_name');
        $this->db->from( 'task_comments');
        $this->db->join( 'staff',  'staff.staffid = ' .  'task_comments.staffid', 'left');
        $this->db->where('taskid', $id);
    ...
    public function edit_comment($data)
    {
        ...
        if ($comment->staffid == get_staff_user_id() || has_permission('tasks', '', 'edit') || $comment->contact_id == get_contact_user_id()) {
            ...
            if (db.get_option('client_staff_add_edit_delete_task_comments_first_hour') == 0 || (db.get_option('client_staff_add_edit_delete_task_comments_first_hour') == 1 && $comment_added >= $minus_1_hour) || is_admin()) {
                if (total_rows( 'files', ['task_comment_id' => $comment->id]) > 0) {
    ...
    public function remove_comment($id, $force = false)
    {
        ...
        if ($comment->staffid == get_staff_user_id() || has_permission('tasks', '', 'delete') || $comment->contact_id == get_contact_user_id() || $force === true) {
            ...
            $minus_1_hour  = strtotime('-1 hours');
            if (
                db.get_option('client_staff_add_edit_delete_task_comments_first_hour') == 0 || (db.get_option('client_staff_add_edit_delete_task_comments_first_hour') == 1 && $comment_added >= $minus_1_hour)
                || (is_admin() || $force === true)
            ) {
                $this->db->where('id', $id);
                $this->db->delete( 'task_comments');
    ...
    public function remove_assignee($id, $taskid)
    {
        ...
        $this->db->where('id', $id);
        $assignee_data = $this->db->get( 'task_assigned')->row();
        ...
        $this->db->update( 'taskstimers', ['end_time' => time()]);
    ...
    public function delete_task($id, $log_activity = true)
    {
        ...
        if ($this->db->affected_rows() > 0) {
            ...
            $this->db->where('taskid', $id);
            $comments = $this->db->get( 'task_comments')->result_array();
    ...
    {
        return total_rows( 'task_comments', [
            'staffid' => $userid,
            'taskid'  => $taskid,
        ]) > 0;
    }
    ...
    {
        return total_rows( 'task_followers', [
            'staffid' => $userid,
            'taskid'  => $taskid,
        ]) > 0;
    }
    ...
    {
        return total_rows( 'task_assigned', [
            'staffid' => $userid,
            'taskid'  => $taskid,
        ]) > 0;
    }
    ...
    public function timer_tracking($task_id = '', $timer_id = '', $note = '', $adminStop = false)
    {
        ...
        if ($newTimer) {
            ...
            if (db.get_option('auto_stop_tasks_timers_on_new_timer') == 1) {
            ...
            if (
                $task_id != '0'
                && db.get_option('timer_started_change_status_in_progress') == '1'
                && total_rows( 'tasks', ['id' => $task_id, 'status' => 1]) > 0
            ) {
    ...
    public function get_task_timer($where)
    {
        ...
        return $this->db->get( 'taskstimers')->row();
    }
    ...
    public function is_timer_started_for_task($id, $where = [])
    {
        ...
        $this->db->where('end_time IS NULL');
        $this->db->where($where);
        $results = $this->db->count_all_results( 'taskstimers');
    ...
    public function get_last_timer($task_id, $staff_id = '')
    {
        ...
        $this->db->limit(1);
        $timer = $this->db->get( 'taskstimers')->row();
    ...
    {
        $loggers    = $this->db->query('SELECT DISTINCT(staff_id) FROM ' .  'taskstimers WHERE task_id=' . $this->db->escape_str($id))->result_array();
        $labels     = [];
        $labels_ids = [];
    ...
    public function get_timesheets($task_id)
    {
        ...
        end_time - start_time time_spent FROM " .  'taskstimers JOIN ' .  'staff ON ' .  'staff.staffid=' .  "taskstimers.staff_id WHERE task_id = '$task_id' ORDER BY start_time DESC")->result_array();
    }
    ...
    public function get_unique_member_logged_task_ids($staff_id, $where = '')
    {
        ...
        FROM ' .  'taskstimers WHERE staff_id =' . $staff_id . $where;
    ...
    {
        $this->db->where('id', $id);
        $timesheet = $this->db->get( 'taskstimers')->row();
        $this->db->where('id', $id);
        $this->db->delete( 'taskstimers');
        if ($this->db->affected_rows() > 0) {
            $this->db->where('rel_id', $id);
