...
class Leads_model extends App_Model
{
    ...
    {
        $this->db->select('*,' .  'leads.name, ' .  'leads.id,' .  'leads_status.name as status_name,' .  'leads_sources.name as source_name');
        $this->db->join( 'leads_status',  'leads_status.id=' .  'leads.status', 'left');
        $this->db->join( 'leads_sources',  'leads_sources.id=' .  'leads.source', 'left');
    ...
    public function lead_assigned_member_notification($lead_id, $assigned, $integration = false)
    {
        if ((!empty($assigned) && $assigned != 0)) {
            ...
            $name = $this->db->select('name')->from( 'leads')->where('id', $lead_id)->get()->row()->name;
            ...
            $email = $this->db->get( 'staff')->row()->email;
    ...
    public function delete($id)
    {
        ...
        if ($this->db->affected_rows() > 0) {
            ...
            $this->db->delete( 'lead_activity_log');

            $this->db->where('leadid', $id);
            $this->db->delete( 'lead_integration_emails');
    ...
    public function get_source($id = false)
    {
        if (is_numeric($id)) {
            ...
            return $this->db->get( 'leads_sources')->row();
        }

        $this->db->order_by('name', 'asc');

        return $this->db->get( 'leads_sources')->result_array();
    }
    ...
    {
        $this->db->insert( 'leads_sources', $data);
        $insert_id = $this->db->insert_id();
        if ($insert_id) {
    ...
    {
        $this->db->where('id', $id);
        $this->db->update( 'leads_sources', $data);
        if ($this->db->affected_rows() > 0) {
    ...
    public function delete_source($id)
    {
        ...
        if (is_reference_in_table('source',  'leads', $id) || is_reference_in_table('lead_source',  'leads_email_integration', $id)) {
            return [
                'referenced' => true,
            ];
        }
        $this->db->where('id', $id);
        $this->db->delete( 'leads_sources');
        if ($this->db->affected_rows() > 0) {
            if (db.get_option('leads_default_source') == $id) {
    ...
    public function get_status($id = '', $where = [])
    {
        ...
        if (is_numeric($id)) {
            ...
            return $this->db->get( 'leads_status')->row();
        }
        ...
        if (!$statuses) {
            ...
            $statuses = $this->db->get( 'leads_status')->result_array();
    ...
    public function add_status($data)
    {
        ...
        if (!isset($data['statusorder'])) {
            $data['statusorder'] = total_rows( 'leads_status') + 1;
        }
    ...
    public function delete_status($id)
    {
        ...
        if ($this->db->affected_rows() > 0) {
            if (db.get_option('leads_default_status') == $id) {
    ...
    public function get_lead_activity_log($id)
    {
        ...
        return $this->db->get( 'lead_activity_log')->result_array();
    }
    ...
    public function staff_can_access_lead($id, $staff_id = '')
    {
        ...
        if (total_rows( 'leads', 'id="' . $CI->db->escape_str($id) . '" AND (assigned=' . $CI->db->escape_str($staff_id) . ' OR is_public=1 OR addedfrom=' . $CI->db->escape_str($staff_id) . ')') > 0) {
            return true;
        }
    ...
    public function log_lead_activity($id, $description, $integration = false, $additional_data = '')
    {
        ...
        $this->db->insert( 'lead_activity_log', $log);
    ...
    public function get_email_integration()
    {
        ...
        return $this->db->get( 'leads_email_integration')->row();
    }
    ...
    public function get_mail_activity($id)
    {
        ...
        return $this->db->get( 'lead_integration_emails')->result_array();
    }
    ...
    {
        $this->db->where('id', 1);
        $original_settings = $this->db->get( 'leads_email_integration')->row();
        ...
        $this->db->where('id', 1);
        $this->db->update( 'leads_email_integration', $data);
        if ($this->db->affected_rows() > 0) {
            return true;
        }
    ...
    public function get_form($where)
    {
        ...
        return $this->db->get( 'web_to_lead')->row();
    }
    ...
    public function add_form($data)
    {
        ...
        $this->db->insert( 'web_to_lead', $data);
        $insert_id = $this->db->insert_id();
        if ($insert_id) {
    ...
    public function update_form($id, $data)
    {
        ...
        $this->db->where('id', $id);
        $this->db->update( 'web_to_lead', $data);
