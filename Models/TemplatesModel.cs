...
public class TemplatesModel(MyInstance self, MyContext db) : MyModel(self,db)
{
  public int create(Template data)
  {
    ...
    hooks.do_action("new_template_added", data.Id);
    return data.Id;
  }
  ...
  {
    var id = data.Id;
    data = hooks.apply_filters("before_template_updated", data);
