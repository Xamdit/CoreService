...
class Knowledge_base_model extends App_Model
{
    ...
    {
        $this->db->select('slug,articleid, articlegroup, subject,' .  'knowledge_base.description,' .  'knowledge_base.active as active_article,' .  'knowledge_base_groups.active as active_group,name as group_name,staff_article');
        $this->db->from( 'knowledge_base');
        $this->db->join( 'knowledge_base_groups',  'knowledge_base_groups.groupid = ' .  'knowledge_base.articlegroup', 'left');
    ...
    public function get_related_articles($current_id, $customers = true)
    {
        ...
        $article = $this->db->get( 'knowledge_base')->row();
    ...
    public function update_groups_order()
    {
        ...
        foreach ($data['order'] as $group) {
            ...
            $this->db->update( 'knowledge_base_groups', [
                'group_order' => $group[1],
            ]);
        }
    }
    ...
    public function get_kbg($id = '', $active = '')
    {
        ...
        if (is_numeric($id)) {
            ...
            return $this->db->get( 'knowledge_base_groups')->row();
        }
        $this->db->order_by('group_order', 'asc');

        return $this->db->get( 'knowledge_base_groups')->result_array();
    }
    ...
    public function add_group($data)
    {
        ...
        $this->db->insert( 'knowledge_base_groups', $data);
        $insert_id = $this->db->insert_id();
        if ($insert_id) {
    ...
    public function update_group($data, $id)
    {
        ...
        $this->db->where('groupid', $id);
        $this->db->update( 'knowledge_base_groups', $data);
        if ($this->db->affected_rows() > 0) {
    ...
    public function delete_group($id)
    {
        ...
        if (is_reference_in_table('articlegroup',  'knowledge_base', $id)) {
            return [
                'referenced' => true,
            ];
        }
        $this->db->where('groupid', $id);
        $this->db->delete( 'knowledge_base_groups');
        if ($this->db->affected_rows() > 0) {
    ...
    public function add_article_answer($articleid, $bool)
    {
        ...
        $answer = $this->db->get( 'knowedge_base_article_feedback')->row();
        ...
        $this->db->insert( 'knowedge_base_article_feedback', [
            'answer'    => $bool,
            'ip'        => $ip,
            'articleid' => $articleid,
