...
class Clients extends AdminController
{
    ...
    public function index()
    {
        ...
        if (!has_permission('customers', '', 'view')) {
            $whereContactsLoggedIn = ' AND userid IN (SELECT customer_id FROM ' .  'customer_admins WHERE staff_id=' . get_staff_user_id() . ')';
        }
    ...
    public function client($id = '')
    {
        ...
        if ($id == '') {
            ...
            if ($group == 'profile') {
                ...
                if (db.get_option('google_api_key') != '' && !empty($client->latitude) && !empty($client->longitude)) {
                    ...
                    $this->app_scripts->add('google-maps-api-js', [
                        'path'       => 'https://maps.googleapis.com/maps/api/js?key=' . db.get_option('google_api_key') . '&callback=initMap',
                        'attributes' => [
                            'async',
                            'defer',
    ...
    public function check_duplicate_customer_name()
    {
        if (has_permission('customers', '', 'create')) {
            ...
            $response    = [
                'exists'  => (bool) total_rows( 'clients', ['company' => $companyName]) > 0,
        ...
        if ($this->input->post()) {
            ...
            if ($contact_id == '') {
                ...
                echo json_encode([
                    'success'             => $success,
                    'message'             => $message,
                    'has_primary_contact' => (total_rows( 'contacts', ['userid' => $customer_id, 'is_primary' => 1]) > 0 ? true : false),
                    'is_individual'       => is_empty_customer_company($customer_id) && total_rows( 'contacts', ['userid' => $customer_id]) == 1,
                ]);
                die;
            }
            if (!has_permission('customers', '', 'edit')) {
    ...
    public function delete_contact($customer_id, $id)
    {
        ...
        $hasProposals = false;
        if ($contact && is_gdpr()) {
            if (total_rows( 'proposals', ['email' => $contact->email]) > 0) {
                $hasProposals = true;
            }
        }
    ...
    public function zip_invoices($id)
    {
        ...
        if (!$has_permission_view && !has_permission('invoices', '', 'view_own')
            && db.get_option('allow_staff_view_invoices_assigned') == '0') {
    ...
    public function zip_estimates($id)
    {
        ...
        if (!$has_permission_view && !has_permission('estimates', '', 'view_own')
            && db.get_option('allow_staff_view_estimates_assigned') == '0') {
    ...
    public function zip_payments($id)
    {
        ...
        $this->app_bulk_pdf_export->set_client_id($id);
        $this->app_bulk_pdf_export->set_client_id_column( 'clients.userid');
        $this->app_bulk_pdf_export->in_folder($this->input->post('file_name'));
        $this->app_bulk_pdf_export->export();
    }
    ...
    public function import()
    {
        ...
        $dbFields = $this->db->list_fields( 'contacts');
        ...
        $dbFields = array_merge($dbFields, $this->db->list_fields( 'clients'));
    ...
    {
        if (!is_admin() && db.get_option('staff_members_create_inline_customer_groups') == '0') {
            access_denied('Customer Groups');
        }
