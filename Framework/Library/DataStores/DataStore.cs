...
public class DataStore : IDataStore
{
  ...
  private readonly BlockingCollection<CommitAction> _updates = new();
  ...
  public void Dispose()
  {
    while (IsUpdating) Task.Run(async () => await Task.Delay(100)).GetAwaiter().GetResult();
  ...
  public dynamic? GetItem(string key)
  {
    ...
    return token == null ? null : SingleDynamicItemReadConverter(token);
  }

  public bool InsertItem<T>(string key, T item)
  {
    return Insert(key, item).Result;
  }

  public async Task<bool> InsertItemAsync<T>(string key, T item)
  {
    return await Insert(key, item, true).ConfigureAwait(false);
  }
  ...
  {
    return Replace(key, item, upsert).Result;
  }

  public async Task<bool> ReplaceItemAsync<T>(string key, T item, bool upsert = false)
  {
    return await Replace(key, item, upsert, true).ConfigureAwait(false);
  }
  ...
  {
    return await Update(key, item, true).ConfigureAwait(false);
  }
  ...
  public IDictionary<string, ValueType> GetKeys(ValueType? typeToGet = null)
  {
    ...
    {
      return (c.Children().FirstOrDefault() is JArray && c.Children().FirstOrDefault().Any() == false)
             || c.Children().FirstOrDefault()?.FirstOrDefault()?.Type == JTokenType.Object;
    }

    bool IsItem(JToken c)
    {
      return c.Children().FirstOrDefault().GetType() != typeof(JArray)
    ...
    lock (_jsonData)
    {
      return typeToGet switch
      {
        null => _jsonData.Children().ToDictionary(c => c.Path, c => IsCollection(c) ? ValueType.Collection : ValueType.Item),
        ValueType.Collection => _jsonData.Children().Where(IsCollection).ToDictionary(c => c.Path, c => ValueType.Collection),
        ValueType.Item => _jsonData.Children().Where(IsItem).ToDictionary(c => c.Path, c => ValueType.Item),
  ...
  private async Task<bool> Commit<T>(string dataPath, Func<List<T>, bool> commitOperation, bool isOperationAsync, Func<JToken, T> readConvert)
  {
    ...
    commitAction.HandleAction = currentJson =>
    {
      ...
      if (!success) return (success, updatedJson);
  ...
  private string GetJsonTextFromFile()
  {
    return FileAccess.ReadJsonFromFile(_filePath, _encryptJson, _decryptJson);
  }

  private JObject GetJsonObjectFromFile()
  {
    return JObject.Parse(GetJsonTextFromFile());
  }
