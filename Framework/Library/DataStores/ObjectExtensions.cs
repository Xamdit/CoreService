...
internal static class ObjectExtensions
{
  ...
  internal static bool IsReferenceType(dynamic? o)
  {
    return IsValueReferenceType(o.GetType());
  }
  ...
  private static void HandleExpandoEnumerable(object source, object destination, dynamic srcProp)
  {
    ...
    for (var i = 0; i < sourceArray.Count; i++)
    {
      ...
      {
        if (targetArray.Count - 1 < i) targetArray.Add(CreateInstance(targetType));
      ...
      {
        if (targetArray.Count - 1 < i) targetArray.Add(new ExpandoObject());
