    ...
    Action<bool> setExecutingState, Func<string, bool> updateState, Func<string> getLatestJson)
  {
    ...
    while (!token.IsCancellationRequested)
    {
      ...
      try
      {
        ...
        while (updates.Count > 0 && batch.Count < CommitBatchMaxSize) batch.Enqueue(updates.Take(token));
      }
      ...
      foreach (var (cbAction, cbSuccess) in callbacks) cbAction.Ready(updateSuccess ? cbSuccess : false, actionException);
